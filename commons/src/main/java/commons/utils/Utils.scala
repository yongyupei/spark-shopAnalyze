...
object ParamUtils{
  def getPageFlow(): Array[String] ={
     var z=Array(1,2,3,4,5,6,7);
     val r=z.slice(0,z.length-1).zip(z.tail).map{
       case (p1,p2)=>{
         p1+"-"+p2;
       }
     }
...
object ValidUtils {
  ...
  def between(data:String, dataField:String, parameter:String, startParamField:String, endParamField:String):Boolean = {
    ...
    val startParamFieldStr = StringUtil.getFieldFromConcatString(parameter, "\\|", startParamField)
    val endParamFieldStr = StringUtil.getFieldFromConcatString(parameter, "\\|", endParamField)
    ...
    val dataFieldStr = StringUtil.getFieldFromConcatString(data, "\\|", dataField)
  ...
  def in(data:String, dataField:String, parameter:String, paramField:String):Boolean = {
    val paramFieldValue = StringUtil.getFieldFromConcatString(parameter, "\\|", paramField)
    if(paramFieldValue == null) {
      return true
    }
    val paramFieldValueSplited = paramFieldValue.split(",")

    val dataFieldValue = StringUtil.getFieldFromConcatString(data, "\\|", dataField)
