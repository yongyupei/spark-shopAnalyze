    ...
    for (i <- 0 to 100) {
      ...
      for (j <- 0 to 10) {
        ...
        for (k <- 0 to random.nextInt(100)) {
          ...
          val actionTime = baseActionTime + ":" + StringUtil.fulfuill(String.valueOf(random.nextInt(59))) + ":" + StringUtil.fulfuill(String.valueOf(random.nextInt(59)))
...
//    spark.sql("DROP TABLE IF EXISTS " + tableName)
    ...
    //dataDF.write.parquet("hdfs://hadoop1:9000/shopAnalyze")
  }
