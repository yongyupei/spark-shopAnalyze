...
object AdUserClickCountDAO {
  def updateBatch1(adUserClickCounts: Array[AdUserClickCount]): Unit ={
    val mySqlPool=CreateMySqlPool();
    val client=mySqlPool.borrowObject();
    val buffer=new StringBuilder();
    var sql = "INSERT INTO ad_user_click_count VALUES"
    buffer.append(sql);
    var index=0;
    for (index<-0 to adUserClickCounts.size-1){
      val action=adUserClickCounts(index);
      var sql1="("+action.date+","+action.userid+","+action.adid+","+action.clickCount+")";
      buffer.append(sql1);
      if (index<adUserClickCounts.size-1)buffer.append(",");
    }
    buffer.append(" on duplicate key update clickCount=values(clickCount)");
    println(buffer.toString());
    client.executeBatch1(buffer.toString());
  }
...
/**
  ...
  *
  */
object AdClickTrendDAO extends Serializable {
